...
const adminNavItems = [
  ...
  { href: '/admin/daily-reports', label: 'メンバー日報', icon: ClipboardList },
  ...
  { href: '/admin/member-mgmt', label: 'メンバー管理', icon: UserPlus },
  { href: '/admin/security', label: 'Security & Proxy', icon: Shield },
]

const inputNavItems = [
  { href: '/input/knowledge', label: '社内知財', icon: Database },
  { href: '/input/field-data', label: '現場生データ', icon: FileAudio },
  { href: '/input/win-loss', label: '勝敗ロジック', icon: Scale },
  { href: '/input/evaluation', label: '評価ロジック', icon: Calculator },
]
...
export function Sidebar() {
  ...
  const showField = user?.role === 'field' || user?.role === 'admin'
  const showInput = isAdmin || isOffice
